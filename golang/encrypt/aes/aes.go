...
var PrivateKey = "1234567812345678"

//var PrivateKey = []byte("1234567812345678")
...
//实现解密
func AesDeCrypt(cypted []byte, ckey string) ([]byte, error) {
	//func AesDeCrypt(cypted []byte, key []byte) ([]byte, error) {
	fmt.Println("==>>> cypted: ", string(cypted))
	key, _ := hex.DecodeString(ckey)
	//创建加密算法的实例
	block, err := aes.NewCipher(key)
	if err != nil {
		return nil, err
